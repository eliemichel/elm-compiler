...
import Data.Monoid ((<>))
import qualified Generate.C.Name as Name
import Generate.C.Name (Name, TypeName)
...
fromStmt level@(Level indent nextLevel) statement =
  case statement of
    ...
    Var typeName name expr ->
      indent <> Name.toBuilder typeName <> " " <> Name.toBuilder name <> " = " <> snd (fromExpr level Whatever expr) <> ";\n"

    Vars typeName [] ->
      mempty

    Vars typeName vars ->
      indent <> Name.toBuilder typeName <> " " <> commaNewlineSep level (map (varToBuilder level) vars) <> ";\n"
